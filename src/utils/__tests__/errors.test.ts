...
describe('mapPostgresError', () => {
  ...
  it('should handle specific Postgres error codes', () => {
    ...
    expect(mapPostgresError({ code: '23503' })).toBe('Não é possível realizar esta ação pois o registro está sendo usado em outro lugar. (Violação de chave estrangeira)');
    expect(mapPostgresError({ code: '23502' })).toBe('Por favor, preencha todos os campos obrigatórios. (Violação de NOT NULL)');
  ...
  it('should handle generic integrity violation class (23)', () => {
    expect(mapPostgresError({ code: '23000' })).toBe('Erro de integridade de dados. Verifique as informações enviadas.');
  });
